...
int rec_way(string& a, string& b, int ind1, int ind2)
{
    ...
    if(a[ind1] == b[ind2])
        return 1 + rec_way(a, b, ind1 - 1, ind2 - 1);
    
    return 0 + max(rec_way(a, b, ind1 - 1, ind2), rec_way(a, b, ind1, ind2 - 1));
}
...
int solve_memoWay(string a, string b, vector<vector<int>>& dp, int ind1, int ind2)
{
    ...
    if(a[ind1] == b[ind2])
        return dp[ind1][ind2] = 1 + solve_memoWay(a, b, dp, ind1 - 1, ind2 - 1);

    return dp[ind1][ind2] = 0 + max(solve_memoWay(a, b, dp, ind1 - 1, ind2), solve_memoWay(a, b, dp, ind1, ind2 - 1));
}
...
{
    vector<vector<int>> dp(a.size() + 1, vector<int>(b.size() + 1, -1)); // +1 beacuse here we have to show -1 also for base case for both index
...
int spaceOp_way(string a, string b)
{
    ...
    for(int j = 0; j <= b.size(); j++) prev[j] = 0;
