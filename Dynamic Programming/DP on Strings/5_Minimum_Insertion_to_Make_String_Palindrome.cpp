// Main idea: find the longes common subsequece between string and rev(string) 
// length(string) - longest common subsequnece is ans
...
int rec_way(string& a, string& b, int ind1, int ind2)
{
    ...
    return 0 + max(rec_way(a, b, ind1 - 1, 0), rec_way(a, b, ind1, ind2 - 1));
}
...
int main()
{
    ...
    // int ans = a.size() - rec_way(a, b, a.size() - 1, b.size() - 1);
    // int ans = a.size() - memo_way(a, b);
    // int ans = a.size() - tabu_way(a, b);
    int ans = a.size() - spaceOp_way(a, b);
    cout<<ans<<endl;
}